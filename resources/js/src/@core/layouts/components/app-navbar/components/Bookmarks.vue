    ...
    <b-nav-item-dropdown link-classes="bookmark-star" lazy @hidden="resetsearchQuery">
      ...
      <li style="min-width:300px">
        ...
        <vue-perfect-scrollbar :settings="perfectScrollbarSettings" class="search-list search-list-bookmark scroll-area"
          :class="{ 'show': filteredData.pages && filteredData.pages.length }" tagname="ul">

          <b-dropdown-item  v-for="(suggestion, index) in filteredData.pages || bookmarks" :key="index"
            class="suggestion-group-suggestion cursor-pointer" link-class="d-flex align-items-center"
            :to="suggestion.route" @mouseenter="currentSelected = index" >
...
function AccesoDirecto() {
  ...
  params.append("idperfil", localStorage.getItem("idPerfil"));
  ...
  axios
    ...
    .then(function (response) {
      ...
      // menuHijo.push({ title: resp[i].name, route: resp[i].route, icon: resp[i].icon, padre: resp[i].parent })
      for (let i = 0; i < resp.length; i++) {
        console.log("Ingresa Con async await dentro de for")
        searchAndBookmarkData.pages.data.push(
          { title: "tes", route: { name: "resp[i].route " }, icon: "resp[i].icon", isBookmarked: true },
        )
      }
